...
from metasynth import fetchSSSDTrainingMask
...
if __name__ == "__main__":
    ...
    # training_samples = []
    # mask_numpy = fetchSSSDTrainingMask(df=training_df, metadata_indices=hierarchical_column_indices, ratio=0.5)
    ...
    out_dim = len(training_df.columns)
    ...
    for epoch in range(args.epochs):
        ...
        for batch in dataloader:
            ...
            num_missing = np.random.randint(0, batch.shape[0]*batch.shape[1]+1)
            signal_mask_indices = np.random.choice(conditional_mask.shape[0]*conditional_mask.shape[1], num_missing, replace=False)
            flat_mask = np.ones(batch.shape[0]*batch.shape[1])
            flat_mask[signal_mask_indices] = 0
            flat_mask_2d = flat_mask.reshape(batch.shape[0], batch.shape[1])
            conditional_mask[:, :, non_hier_cols] = flat_mask_2d[:, :, np.newaxis]
            ...
            sigmas_predicted = model((batch_noised, batch, conditional_mask), timesteps).permute((0, 2, 1))
            optimizer.zero_grad()
            boolmask = conditional_mask.bool()
            loss = criterion(sigmas_predicted[~boolmask], sigmas[~boolmask])
            loss.backward()
            total_loss += loss
            optimizer.step()
    ...
    if args.propCycEnc:
        filename = "model_prop_sssd_signalconditioned.pth"
    else:
        filename = "model_sssd_signalconditioned.pth"
