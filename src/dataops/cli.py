...
cli — optional convenience wrapper for standalone runs.
    ...
    python -m dataops manifest
    python -m dataops profile     --input data.csv            --output profile.json
    python -m dataops ts-qc       --input data.csv            --output report.json  --ts-col time
    python -m dataops tabular-qc  --input data.csv            --output report.json
    python -m dataops preprocess  --input raw.csv             --prepared-dir ./prepared_amf
    python -m dataops split       --prepared-dir ./prepared_amf

``--input/--output/--prepared-dir`` accept a local path or ``s3://bucket/key``.
This file is a convenience wrapper around the pure modules. New integrations
should prefer the public ``data_process_modules`` package; ``dataops`` remains
the core/backward-compatible implementation path.
"""
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="dataops", description="6G-DALI DataOps tools")
