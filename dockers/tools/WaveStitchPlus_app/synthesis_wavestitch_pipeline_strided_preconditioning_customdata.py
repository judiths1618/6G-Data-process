...
def load_custom_prepared(prepared_dir: str):
    ...
    # time_col = meta.get("time_col", "time_stamp")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-dataset", "-d", type=str,
        help="custom_csv, MetroTraffic, BeijingAirQuality, AustraliaTourism, WebTraffic, StoreItems",
        required=True
    )
    ...
    if dataset == "custom_csv":
        ...
        hierarchical_column_indices = np.array([model_cols.index(c) for c in cond_cols if c in model_cols], dtype=int)
        ...
        # 🔥 修复：正确构建窗口 [N, W, C]
        m_vals = rows_to_synth.astype(np.bool_)                                # [T]
        ...
        for i in range(num_windows):
            ...
            windows[i] = torch.from_numpy(d_vals_raw[start:end, :])  # [W, C]
            masks[i] = torch.from_numpy(m_vals[start:end])       # [W]
        
        print(f"[INFO] Windows shape: {windows.shape} (should be [N, W, C])")
        print(f"[INFO] Masks shape: {masks.shape} (should be [N, W])")
    ...
    # 2) Dataset & Model
    # -----------------------------
    in_dim = windows.shape[2]  # C (channels last)
    out_dim = in_dim - len(hierarchical_column_indices)

    print(f"[INFO] in_dim={in_dim}, out_dim={out_dim}, hierarchical_indices={hierarchical_column_indices}")
    ...
    # model_path = f"saved_models/{args.dataset}/model_prop.pth" if args.propCycEnc else f"saved_models/{args.dataset}/model.pth"
    model_path=f"{saved_dir}/model_prop.pth" if args.propCycEnc else f"{saved_dir}/model.pth"
    ...
    for trial in range(args.n_trials):
        ...
        with torch.no_grad():
            ...
            for idx, (test_batch, mask_batch) in enumerate(zip(test_dataloader, mask_dataloader)):
                ...
                for step in range(diffusion_config["T"] - 1, -1, -1):
                    ...
                    x[:, :, non_hier_cols] = x[:, :, non_hier_cols] + (-0.1 * grad[:, :, non_hier_cols])
        ...
        if dataset == "custom_csv":
            ...
            if scaler_mean is not None and scaler_std is not None:
                ...
                print(f"[WARNING] No scaler available, outputting raw values")
            ...
            # out_dir = f"generated/{args.dataset}/{args.synth_mask}/"
            # out_dir = f"{get_generated_dir(args.prepared_dir)}/{args.synth_mask}/"
            ...
            # print(f"[custom_csv] Trial {trial} wrote: {out_name}")

    # out_dir = f"generated/{args.dataset}/{args.synth_mask}/"
    out_dir = f"{get_generated_dir(args.prepared_dir)}/{args.synth_mask}/"
            
    os.makedirs(out_dir, exist_ok=True)
    timing_file = os.path.join(out_dir, f"denoiser_calls_pipeline_stride_{args.stride}_cycStd_grad_correction.txt")
    with open(timing_file, "a") as f:
        arr = np.array(exec_times)
        f.write("\n" + str(np.mean(arr)) + "\n")
        f.write(str(np.std(arr)))
