...
#     """Torch version of soft_clip (for use inside the model loop)."""
...
#         """
#         Args:
#             mode: "standard" (mean/std) or "robust" (median/IQR)
#             clip_mode: "hard" or "soft"
#             clip_bound: hard clip range [-b, +b]  (only for hard mode)
#             soft_bound: tanh saturation bound     (only for soft mode)
#         """
#         self.mode = mode
...
#         self.center_ = center.copy()
#         self.scale_ = scale.copy()
...
#         """Bound to use for clamping x_0 predictions inside the model."""
#         if self.clip_mode == "soft":
...
#         # Backward-compat aliases for synthesize.py
...
#         else:
#             # Backward compat: old-style mean.npy / std.npy
#             scaler.center_ = np.load(os.path.join(directory, "mean.npy"))
#             scaler.scale_ = np.load(os.path.join(directory, "std.npy"))
...
#                 # col_masks cover the full regularized set;
#                 # train is the first split_ratio fraction
...
#     # ── M-step (annealed missing weight) ────────────────────────────
...
#                     os.path.join(save_dir, "model_em_best.pth"),
#                 )
...
#                     batch_size=e_step_batch_size, num_samples=1,
...
#                 p = os.path.join(save_dir, f"model_em_iter{it+1}.pth")
...
#     # Needed later for completion metadata; set defaults for non-custom
...
#         if args.input_csv:
#             preprocess_csv(
#                 input_csv=args.input_csv, output_dir=prepared_dir,
#                 time_col=None, base_dt=None,
...
#                 mode=scaler_mode, clip_mode=clip_mode,
#                 clip_bound=args.clip_bound, soft_bound=args.soft_bound,
#             )
...
#                     center = np.array([stats[c]["median"] for c in target_cols], dtype=np.float32)
#                     scale = np.array([stats[c]["iqr"] for c in target_cols], dtype=np.float32)
#                 else:
#                     center = np.array([stats[c]["mean"] for c in target_cols], dtype=np.float32)
#                     scale = np.array([stats[c]["std"] for c in target_cols], dtype=np.float32)
#             else:
...
#             non_hier_cols=non_hier_cols, hier_cols=hierarchical_column_indices,
#             target_indices=target_indices, lr=args.lr,
#             em_iterations=args.em_iterations, repaint_rounds=repaint_rounds,
#             clip_bound=model_clip_bound,
#         )

#         final_data = trainer.train_em(
#             data_np=d_vals, obs_mask_np=obs_mask_np,
#             window_size=args.window_size, stride=args.stride,
#             batch_size=args.batch_size, epochs_per_iter=args.epochs_per_em,
#             e_step_batch_size=args.e_step_batch_size,
#             ddim_steps=args.ddim_steps, save_dir=save_dir,
#         )

#         np.save(os.path.join(prepared_dir, "train_imputed.npy"), final_data)
#         torch.save(model.state_dict(), os.path.join(save_dir, "model_em.pth"))
        ...
        if args.normalize:
            ...
            if os.path.exists(stats_path):
                ...
                if scaler_mode == "robust":
                    ...
                    scale = np.array(
                        [stats[c]["iqr"] for c in target_cols],
                        dtype=np.float32
                    )
                else:
                    center = np.array(
