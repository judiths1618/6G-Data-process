...
class MyDataset(Dataset):
    """
    Window inputs: ensure each sample returned as [W, C]
    Mask inputs: return [W]
    """
    def __init__(self, inputs, window_size: int = None):
        self.inputs = inputs
    ...
    def __getitem__(self, idx):
        ...
        # window case: [A, B] -> force [W, C]
        if x.ndim == 2 and self.window_size is not None:
            a, b = x.shape
            if a == self.window_size:      # already [W, C]
                return x
            if b == self.window_size:      # [C, W] -> [W, C]
                return x.transpose(0, 1)
            raise ValueError(f"Cannot infer window axis: {x.shape}, window_size={self.window_size}")
