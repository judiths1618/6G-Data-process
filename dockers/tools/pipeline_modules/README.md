...
checks, preprocessing/transformation, and train/test splitting. These are the
non-imputation building blocks for an end-to-end pipeline; the imputation step
is provided by the sibling apps (`Darts_app`, `ImputeGAP_app`, `PyPOTS_app`,
`WaveStitchPlus_app`).
...
existing local prepared-bundle writer so current imputation apps keep their
artifact contract.
...
aggregates them so a host system can enumerate, validate params, and chain them.
...
through `io_utils` (env: `S3_ENDPOINT/S3_ACCESS_KEY/S3_SECRET_KEY`).
`preprocess` is local-path only for now because it writes the complete prepared
bundle in one call.
...
`tests/` covers the pure logic exhaustively — profiling (TS/PK detection),
`split` (including a **bit-for-bit reproduction** check against the real
`eval_holdout_mask.npy` for **all four** EUR subsets), gap detection, the
metadata manifest, and local I/O. `test_subsets.py` additionally runs profiling
and gap detection against the four real raw datasets
...
gated on the **GX 0.18** fluent API the modules target (the project pin), so
they skip in envs carrying GX 1.x and run in the Airflow image / a GX-0.18 venv.
...
Logic is extracted from the existing pipeline so behavior remains aligned:
