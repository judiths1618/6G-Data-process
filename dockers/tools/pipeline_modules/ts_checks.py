...
GX runs through the no-Airflow :func:`pipeline_modules.gx.get_gx_context` shim. Pure:
...
def run(
    ...
    *,
    ...
    asset = ds.add_dataframe_asset(name="ts_asset", dataframe=df)
    ...
    numeric_cols = [
        c for c in df.select_dtypes(include="number").columns if c != ts_col
    ]
    outlier_cols = []
    for col in numeric_cols:
        ...
        validator.expect_column_values_to_be_between(
            column=col, min_value=lower, max_value=upper, mostly=1 - 2 * outlier_q
        )
