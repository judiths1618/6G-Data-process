...
:func:`pipeline_modules.gx.get_gx_context` shim. Pure: ``run(df) -> dict`` (field-compatible
...
def run(
    ...
    *,
    ...
    asset = ds.add_dataframe_asset(name="tabular_asset", dataframe=df)
    batch_request = asset.build_batch_request()
    suite = ctx.add_or_update_expectation_suite("tabular_quality")
    ...
    for col in df.columns:
        ...
        suite.expect_column_values_to_not_be_null(column=col, mostly=mostly)
    ...
    for col in numeric_cols:
        ...
        suite.expect_column_values_to_be_between(
            column=col, min_value=q_low, max_value=q_high, mostly=1 - 2 * outlier_q
        )
...
METADATA = {
    ...
    "entrypoint": "pipeline_modules.tabular_checks:run",
    "gpu": False,
